...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def losses(self, seg_logits, seg_label):
        ...
        for seg_logit in seg_logits:
            # seg_logit = resize(input=seg_logit,size=size,mode='bilinear',align_corners=self.align_corners)

            if loss_decode.loss_name not in loss:
                loss[loss_decode.loss_name] = loss_decode(seg_logit,seg_label,weight=seg_weight,ignore_index=self.ignore_index)
            else:
                loss[loss_decode.loss_name]  =  loss[loss_decode.loss_name].clone().detach()+ loss_decode(seg_logit,seg_label,weight=seg_weight,ignore_index=self.ignore_index)
