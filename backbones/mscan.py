...
class AttentionModule(BaseModule):      #MSCA
...
class MSCAN(BaseModule):
    ...
    def forward(self, x):
        # cv2.imwrite("/media/xia/40903229-3d5d-4272-81a0-93b4ad6abb57/WJH/SegNeXt-main/results/png",
        #             x.cpu().numpy())
        B = x.shape[0]
        outs = []
        outs.append(x)
        for i in range(self.num_stages):
            ...
            x, H, W = patch_embed(x)        #1*102400*64
            for blk in block:
                x = blk(x, H, W)
            x = norm(x)
