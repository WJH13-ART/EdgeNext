...
class RandomCrop(object):
    ...
    #     img, mask = self.randomVerticleFlip(img, mask)
    #     if mask.shape[0] > self.crop_size + 12 and mask.shape[1] > self.crop_size + 12:
    #         buf = 5
    #         x_base = np.random.randint(buf, mask.shape[1] - self.crop_size - buf)
    #         y_base = np.random.randint(buf, mask.shape[0] - self.crop_size - buf)
    #         img = img[y_base: y_base + self.crop_size, x_base:x_base + self.crop_size, :]
    #         mask = mask[y_base: y_base + self.crop_size, x_base:x_base + self.crop_size]
    #     else:
